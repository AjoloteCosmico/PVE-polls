...
query = """select * from respuestas20 where gen_dgae=2022 and completed=1 and aplica in ('27','28','29')""" # Modificación para filtrar solo las respuestas de licenciatura

encuestas=pd.read_sql(query,engine)
for col in ['created_at','updated_at','fec_capt']:
    encuestas[col]=encuestas[col].astype(str)
encuestas.to_excel('storage/base22AAA.xlsx')