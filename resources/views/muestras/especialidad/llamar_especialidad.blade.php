...
<div class="container-fluid cuadro-azul">
    <div>
        <table class="cuadro-azul">
            <tr>
                <td>
                    <h3>{{$EgresadoEsp->nombre}} {{$EgresadoEsp->paterno}} {{$EgresadoEsp->materno}}</h3>
                </td>
                <td>
                    <h3>{{$EgresadoEsp->cuenta}}  </h3>
                </td>
            </tr>
            <tr>
                <td>   
                    <h3>ESP. EN DERECHO {{$EgresadoEsp->especialidad}}  </h3> 
                    <h4>{{$EgresadoEsp->plantel}}  </h4> 
                </td>
                <td>
            ...
            <tr>
                @php
                    $codigoStatus = $Codigos_all->where('code',$EgresadoEsp->status)->first();
                @endphp
                    ...
                    @include('recados.recado_form_content')
                        ...
                        <div class="row tel-contorno-div">
                            <div class="col tel-contorno-div">
                                 <a href="{{route('act_data_especialidad',[$EgresadoEsp->cuenta,$EgresadoEsp->especialidad,$telefono->id])}}">
    ...
    <div class='row'>
        <div class='col'>
            <a href="{{route('muestras.especialidad.show',[$EgresadoEsp->especialidad])}}">
                <button type="button"  class="boton-oscuro">
                    <i class="fas fa-table"></i> Ir a muestra Especialidad
                </button>
            </a>
        </div>
    </div>
