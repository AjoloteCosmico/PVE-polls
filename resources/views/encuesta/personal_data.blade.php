<table class="table text-lg personal" >
  <tr>
    ...
    <th> Telefonos: 
      <a href="{{route('agregar_telefono',[$Egresado->cuenta,$Egresado->carrera,$Encuesta->registro,Session::get('telefono_encuesta')])}}">
        <button class="btn btn-mb2" style="background-color:{{Auth::user()->color}} ; color:white; font-size:0.9vw">
    ...
    <td>
      <a class="contact_data"  href="{{route('editar_telefono',[$t->id,$Egresado->carrera,$Encuesta->registro,Session::get('telefono_encuesta')])}}">
        {{$t->telefono}}
      </a>
    </td>
    @endforeach
    <th>Promedio:</th>
      <td>@if($Egresado->promedio>10) {{$Egresado->promedio /100}} @else {{$Egresado->promedio}} @endif</td>
    <th>fec. nac.:</th>
      <td>{{$Egresado->fec_nac}}</td>
  </tr>
  <tr>
    <th>Carrera:</th>
    <td> {{$Carrera}}</td> 
    ...
    <td colspan="{{$Telefonos->count()}}" >
      ...
      <a class="contact_data" onclick="correos({{$c->id}},'{{$c->correo}}',{{Session::get('telefono_encuesta')}})">
        {{$c->correo}}
      </a>&nbsp;
      @endforeach
      <a href="{{route('agregar_correo',[$Egresado->cuenta,$Egresado->carrera,$Encuesta->registro,Session::get('telefono_encuesta')])}}">
        <button class="btn btn-mb2" style="background-color:{{Auth::user()->color}} ; color:white; font-size:0.9vw">
...
<script type="text/javascript">
  ...
  function correos(correo_id,correo,telefono){
    ...
    Swal.fire({
      ...
      html: 
            ...
            '<button type="button" role="button" tabindex="0" class="SwalBtn2 customSwalBtn"  onclick="location.href = `/editar_correo/'+correo_id +'/{{$Egresado->carrera}}/{{$Encuesta->registro}}/'+telefono+'`">' + 'Editar' + '</button>',
      icon: "warning",
